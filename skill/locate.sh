...
#   3. 当前目录或父目录里的 _scripts/artvault.py
#   4. 常见 clone 位置
set -e

# 1) 环境变量
if [ -n "$ARTVAULT" ] && [ -f "$ARTVAULT/_scripts/artvault.py" ]; then
...
for cand in "$(dirname "$SELF")" "$(dirname "$(dirname "$SELF")")"; do
  if [ -f "$cand/_scripts/artvault.py" ]; then echo "$cand"; exit 0; fi
done

# 3) 从当前目录往上找
d="$PWD"
while [ "$d" != "/" ]; do
  if [ -f "$d/_scripts/artvault.py" ]; then echo "$d"; exit 0; fi
  d="$(dirname "$d")"
done
