...
def interpret(d):
    ...
    **重要**：实测发现 Vision 特征对「艺术流派」几乎没有区分度 ——
    最近邻同流派命中率只有 44%（近乎随机），同流派与跨流派的距离分布
    几乎重合（中位 1.08 vs 1.05）。原因是 VNGenerateImageFeaturePrint
    是为**物体/场景识别**训练的（同一个建筑、同一只狗），不是为艺术风格。

    所以这个距离只能可靠地判断「是不是同一张/同一构图」，
    不能判断「是不是同一流派」。阈值按前者标定：

        <0.10  几乎同一张（真重复，实测 17 对全是这种方式找出来的）
        <0.35  高度相似（同题材、同构图）
        <0.60  明显相似
        <0.85  有一定共同点
        其余   不构成相似
...
def cmd_similar(slug, topn):
    ...
    print()
    print("⚠ 这个排序只能当**弱参考**：实测 Vision 特征判「同一流派」的最近邻命中率")
    print("  只有 44%（近乎随机），它认的是物体和构图，不是艺术风格。")
    print("  要判断风格接近，用 `artvault.py related <流派>`（基于文字关联）更可靠。")
    return 0
